...
// node_modules/.pnpm/highcharts@12.5.0/node_modules/highcharts/modules/offline-exporting.js
var require_offline_exporting = __commonJS({
  "node_modules/.pnpm/highcharts@12.5.0/node_modules/highcharts/modules/offline-exporting.js"(exports, module) {
    !/**
    ...
    */
    (function(t, e) {
      ...
      i.n = (t2) => {
        ...
        for (var o2 in e2) i.o(e2, o2) && !i.o(t2, o2) && Object.defineProperty(t2, o2, { enumerable: true, get: e2[o2] });
      }, i.o = (t2, e2) => Object.prototype.hasOwnProperty.call(t2, e2);
      var l = {};
      i.d(l, { default: () => N });
      var s = i(944), c = i.n(s);
      let { isSafari: d, win: f, win: { document: h } } = c(), { error: p } = c(), u = f.URL || f.webkitURL || f;
      function g(t2) {
        ...
        if (e2 && e2.length > 3 && f.atob && f.ArrayBuffer && f.Uint8Array && f.Blob && u.createObjectURL) {
          ...
          return u.createObjectURL(new f.Blob([r2], { type: e2[1] }));
        }
      }
      function y(t2, e2) {
      ...
      let E = { exporting: {} }, { getOptions: F, setOptions: A } = c(), { composed: S, doc: j, win: C } = c(), { addEvent: H, extend: L, pushUnique: B } = c();
      !(function(t2) {
        async function e2(t3, e3, n2, a2) {
          var i2, l2;
          ...
          p2 && (await o2(p2, a2), y(await r2(p2, 0, e3), n2));
        }
        async function o2(t3, e3) {
          let o3, r3, n2 = (t4, e4) => {
          ...
          for (let a2 of (e3 && (r3 = t3.textContent || "", !/[^\u0000-\u007F\u200B]+/.test(r3)) && (e3 = void 0), ["normal", "italic", "bold", "bolditalic"])) {
            let t4 = e3?.[a2];
            if (t4) try {
              let e4 = await C.fetch(t4);
              if (!e4.ok) throw Error(`Failed to fetch font: ${t4}`);
              let r4 = await e4.blob(), i2 = new FileReader(), l2 = await new Promise((t5, e5) => {
                i2.onloadend = () => {
        ...
        async function r2(t3, e3, o3) {
          ...
          }), await a2.svg(t3, { x: 0, y: 0, width: r3, height: n2, removeInvalid: true }), a2.output("datauristring");
        }
        t2.compose = function(t3) {
          if (H(t3, "downloadSVG", async function(t4) {
            let { svg: o4, exportingOptions: r3, exporting: n2, preventDefault: a2 } = t4;
            if (r3?.type === "application/pdf") {
              a2?.();
              try {
                let { type: t5, filename: n3, scale: a3, libURL: i2 } = c().Exporting.prepareImageOptions(r3);
                "application/pdf" === t5 && (C.jspdf?.jsPDF || (await m(`${i2}jspdf.js`), await m(`${i2}svg2pdf.js`)), await e2(o4, a3, n3, r3?.pdfFont));
              } catch (t5) {
                await n2?.fallbackToServer(r3, t5);
              }
            }
          }), !B(S, "OfflineExporting")) return;
          L(x().prototype, { exportChartLocal: async function(t4, e3) {
            await this.exporting?.exportChart(t4, e3);
          } }), A(E);
          let o3 = F().exporting?.buttons?.contextButton?.menuItems;
          o3 && o3.push("downloadPDF");
        }, t2.downloadSVGLocal = async function(t3, e3) {
          await c().Exporting.prototype.downloadSVG.call(void 0, t3, e3);
        };
      })(r || (r = {}));
      let O = r, U = c();
