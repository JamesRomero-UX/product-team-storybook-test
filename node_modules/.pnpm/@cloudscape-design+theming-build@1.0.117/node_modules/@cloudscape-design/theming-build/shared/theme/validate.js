...
exports.getThemeFromPreset = exports.getContexts = exports.validateOverride = void 0;
...
function validateOverride(override, themeable, availableContexts) {
    ...
    const unthemeableTokenWarningCache = {};
    function isThemeable(token) {
        const isThemeable = (0, utils_1.includes)(themeable, token);
    ...
    function isValidContextId(contextId) {
        const isValid = (0, utils_1.includes)(availableContexts, contextId);
        if (!isValid) {
            console.warn(`${contextId} is not a valid ID of a visual context and will be ignored during theming.`);
        }
        return isValid;
    }
    const tokensEntries = (0, utils_1.entries)(override.tokens).filter(([token]) => isThemeable(token));
    const contextEntries = (override.contexts
        ? (0, utils_1.entries)(override.contexts).filter(([, context]) => context !== undefined)
        : [])
        .filter(([contextId]) => isValidContextId(contextId))
        .map(([contextId, context]) => {
        const filteredTokens = (0, utils_1.entries)(context.tokens).filter(([token]) => isThemeable(token));
        const from = (0, utils_1.fromEntries)(filteredTokens);
        const newContext = {
            ...context,
            tokens: from,
        };
    ...
    if (override.referenceTokens?.color) {
        const generatedTokens = (0, process_1.processReferenceTokens)(override.referenceTokens.color);
        // Reference tokens should override existing tokens
        completeTokens = { ...(0, utils_1.fromEntries)(tokensEntries), ...generatedTokens };
    }
    else {
        completeTokens = (0, utils_1.fromEntries)(tokensEntries);
    }
    return {
        contexts: (0, utils_1.fromEntries)(contextEntries),
...
function getThemeFromPreset(preset, baseThemeId) {
    ...
    const themesMap = [preset.theme, ...(preset.secondary ?? [])].reduce((accThemesMap, currentTheme) => {
        accThemesMap[currentTheme.id] = currentTheme;
        return accThemesMap;
    }, {});
    if (!themesMap[baseThemeId]) {
        throw new Error(`Specified baseThemeId '${baseThemeId}' is not available. Available values are ${Object.keys(themesMap)
            .map((value) => `'${value}'`)
            .join(', ')}.`);
    }
    return themesMap[baseThemeId];
}
exports.getThemeFromPreset = getThemeFromPreset;
