...
export class Processor<TState, TInputEvents, TOutputEvents> {
  rules: Map<
    RecordTypeName,
    StateUpdater<TState, TInputEvents, TOutputEvents, TInputEvents>
  >;
  ...
  process(
    state: TState | null,
    pastInboundEvents: Array<Event<TInputEvents>> = new Array<
      Event<TInputEvents>
    >(),
    newInboundEvents: Array<Event<TInputEvents>> = new Array<
      Event<TInputEvents>
    >()
      ...
      if (updater) {
        result.state = updater({
          ...
          publish: (eventName: string, event: TOutputEvents) => {
            ...
            if (idx >= pastInboundEvents.length) {
