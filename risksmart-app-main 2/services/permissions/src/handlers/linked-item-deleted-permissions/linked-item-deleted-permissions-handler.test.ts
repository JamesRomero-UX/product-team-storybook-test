...
import type { LinkedItemDeletedPermissionsDependencies } from './linked-item-deleted-permissions-handler';
import { createLinkedItemDeletedPermissionsHandler } from './linked-item-deleted-permissions-handler';

describe('LinkedItemDeleted permissions handler', () => {
  ...
  const mockDeps: LinkedItemDeletedPermissionsDependencies = {
  ...
  it('should skip sibling relationships', async () => {
    const handler = createLinkedItemDeletedPermissionsHandler(mockDeps);
    await handler({
  ...
  it('should process ParentChild relationship correctly', async () => {
    mockGetNode
  ...
  it('should not process if both source and target nodes not found', async () => {
    mockGetNode
      .mockResolvedValueOnce(null) // source node
      .mockResolvedValueOnce(null); // target node
