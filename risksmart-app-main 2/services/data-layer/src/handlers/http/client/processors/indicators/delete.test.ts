...
vi.mock('../../../../../repositories/indicator-repository', () => ({
  createIndicatorRepository: vi.fn(),
}));
...
const createMockDependencies = () => {
  ...
  const dependencies: ProcessorDependencies = {
    ...
    } as unknown as ProcessorDependencies['indicatorRepository'],
  };
...
describe('DELETE /indicators (batch)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('createProcessor', () => {
    it('should delete indicators and return deleted IDs on success', async () => {
      // Arrange
    ...
    it('should throw NotFound when no IDs were deleted', async () => {
      ...
      await expect(processor({ payload, context })).rejects.toThrow(
        'None of the specified indicators were found'
      );
    });
