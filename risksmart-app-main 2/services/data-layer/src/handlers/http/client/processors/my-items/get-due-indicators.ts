...
import type { GetMyDueItemsIndicatorsResponseRow } from '../../../../../types';
...
export const myDueIndicatorsQueryConfig = z
  .object({
...
/**
 * Processor for GET /my-items/due-indicators
 * Retrieves due indicators for the current user
 *
 * Query parameters:
 * - date: string representing the cutoff date for due indicators
 * - userId: string representing the user to filter ownership for
 * - owner, contributor, groupOwner, etc.: ownership filter flags
 *
 * Example: GET /my-items/due-indicators
 * Example: GET /my-items/due-indicators?date=2024-06-30&userId=123&owner=true
 */
export const getMyDueIndicators = async (
  ...
  >()
    .withQueryParamsSchema(myDueIndicatorsQueryConfig)
    .withObjectName('Due indicator')
    ...
    .withHandler(async ({ queryParams, serviceContext }) => {
      ...
      return await myItemsRepository.getDueIndicators(
        queryParams.date,
        queryParams
      );
    })
