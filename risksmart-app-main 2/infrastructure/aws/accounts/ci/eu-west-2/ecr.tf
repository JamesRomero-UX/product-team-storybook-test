...
#### ai engine chat-api api ####

resource "aws_ecr_repository" "chat_api" {
  name                 = var.repository_name_chat_api
  image_tag_mutability = "IMMUTABLE"
  image_scanning_configuration {
    scan_on_push = true
  }
...
resource "aws_ecr_lifecycle_policy" "chat_api" {
  ...
  policy = jsonencode({
    rules = [
      {
        "rulePriority" : 1,
        "description" : "Delete PR images after 7 days",
        "selection" : {
          "tagStatus" : "tagged",
          "tagPrefixList" : ["pr-"],
          "countType" : "sinceImagePushed",
      ...
      {
        "rulePriority" : 2,
        "description" : "Keep only 20 main versioned images",
        "selection" : {
          "tagStatus" : "tagged",
          "tagPrefixList" : ["v"],
          "countType" : "imageCountMoreThan",
...
resource "aws_ecr_repository_policy" "chat_api" {
  repository = aws_ecr_repository.chat_api.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
#### ai engine workflow-api api ####

resource "aws_ecr_repository" "workflow_api" {
  name                 = var.repository_name_workflow_api
  image_tag_mutability = "IMMUTABLE"
  image_scanning_configuration {
    scan_on_push = true
  }
...
resource "aws_ecr_lifecycle_policy" "workflow_api" {
  repository = aws_ecr_repository.workflow_api.name
  policy = jsonencode({
    rules = [
      {
        "rulePriority" : 1,
...
resource "aws_ecr_repository_policy" "workflow_api" {
  repository = aws_ecr_repository.workflow_api.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
#### hybiscusdev managed-cloud api ####

resource "aws_ecr_repository" "hybiscusdev_managed_cloud_api" {
  name                 = var.repository_name_hybiscus
  image_tag_mutability = "IMMUTABLE"
  image_scanning_configuration {
    scan_on_push = true
  }
...
resource "aws_ecr_lifecycle_policy" "hybiscusdev_managed_cloud_api" {
  repository = aws_ecr_repository.hybiscusdev_managed_cloud_api.name
  policy = jsonencode({
    rules = [
      {
        rulePriority = 1
...
resource "aws_ecr_repository_policy" "hybiscusdev_managed_cloud_api" {
  repository = aws_ecr_repository.hybiscusdev_managed_cloud_api.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "aws_ecr_repository" "auth0_mcp" {
  name                 = var.repository_name_auth0_mcp
  image_tag_mutability = "IMMUTABLE"
  image_scanning_configuration {
    scan_on_push = true
  }
...
resource "aws_ecr_lifecycle_policy" "auth0_mcp" {
  repository = aws_ecr_repository.auth0_mcp.name
  policy = jsonencode({
    rules = [
      {
        "rulePriority" : 1,
...
resource "aws_ecr_repository_policy" "auth0_mcp" {
  repository = aws_ecr_repository.auth0_mcp.name
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
