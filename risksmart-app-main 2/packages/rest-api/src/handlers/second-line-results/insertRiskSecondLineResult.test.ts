...
import { handler } from './insertRiskSecondLineResult';
...
const insertRiskControlledSecondLineResultsMock = vi.mocked(
  insertRiskControlledSecondLineResults
);
const insertRiskUncontrolledSecondLineResultsMock = vi.mocked(
  insertRiskUncontrolledSecondLineResults
);
...
describe('insert risk rating second line result - post', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  it('returns bad request when inserting risk rating fails', async () => {
    ...
    insertRiskControlledSecondLineResultsMock.mockResolvedValue(undefined);
    insertRiskUncontrolledSecondLineResultsMock.mockResolvedValue(undefined);
  ...
  it('returns 200 when items inserted correctly - Controlled', async () => {
    ...
    hasPermissionMock.mockResolvedValue(true);
    insertRiskControlledSecondLineResultsMock.mockResolvedValue(1);
    ...
    expect(insertRiskControlledSecondLineResultsMock).toHaveBeenCalledTimes(1);
    expect(insertRiskControlledSecondLineResultsMock).toHaveBeenCalledWith(
      hasuraMock,
      {
        results: [
          {
  ...
  it('returns 200 when items inserted correctly - Uncontrolled', async () => {
    ...
    hasPermissionMock.mockResolvedValue(true);
    insertRiskUncontrolledSecondLineResultsMock.mockResolvedValue(1);
    randomUUIDMock.mockReturnValue('3cf6e4fe-82df-454b-a7b6-4f11611ba504');
    getSecondLineReportMock.mockResolvedValue({
      compliance_monitoring_assessment: [
        {
    ...
    expect(insertRiskUncontrolledSecondLineResultsMock).toHaveBeenCalledTimes(
      1
    );
    expect(insertRiskUncontrolledSecondLineResultsMock).toHaveBeenCalledWith(
      hasuraMock,
      {
        results: [
          {
