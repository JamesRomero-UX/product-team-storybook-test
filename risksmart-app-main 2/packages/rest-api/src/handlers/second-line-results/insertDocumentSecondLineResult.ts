...
import type { DocumentSecondLineResultInsertInput } from 'generated/graphql';
...
import { DocumentSecondLineResultSchema } from './schema';

export const handler = backendRouteHandler(
  DocumentSecondLineResultSchema,
  async (body) => {
    ...
    const Ids = [];
    const results: DocumentSecondLineResultInsertInput[] = [];
    for (const documentId of input.DocumentIds) {
      const Id = crypto.randomUUID();
      Ids.push(Id);
      const result: DocumentSecondLineResultInsertInput = {
        Id: Id,
        Rating: input.Rating,
    ...
    const result = await insertDocumentSecondLineResult(hasuraClient, {
      results,
    });
