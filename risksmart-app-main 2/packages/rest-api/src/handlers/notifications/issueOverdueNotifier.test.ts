...
import { handler } from './issueOverdueNotifier';
...
describe('issueOverdueNotifier', () => {
  ...
  describe('no_inherit feature flag', () => {
    const createIssueOverdueEvent = () =>
      stub<EventBridgeEvent<string, IssueOverdueEventDetail>>({
        'detail-type': 'IssueOverdue',
        detail: {
          meta: {
            tenant: 'test-tenant',
          },
    ...
    it('should use direct parent contributors when no_inherit feature flag is enabled', async () => {
      ...
      await handler(createIssueOverdueEvent(), stub<Context>({}), vi.fn());
