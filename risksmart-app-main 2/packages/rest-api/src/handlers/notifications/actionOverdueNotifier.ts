...
import type { RisksmartDetailType } from './eventBridgeUtils';
import {
...
const workflowKey = 'action-overdue';
...
>(async (e) => {
  ...
  const idempotencyKey = `${workflowKey}-${action.Id}-${action.DateDue}`;
  ...
  logger.info('Got org details, getting receipients');
