...
export const handler = backendRouteHandler(PostSchema, async (body) => {
  ...
  const id = randomUUID();
  const result = await apiClient.insertComplianceMonitoringAssessment({
    Id: id,
    Title: input.Title,
  ...
  const assessmentId = result.insert_compliance_monitoring_assessment_one?.Id;
  if (!assessmentId) {
    throw new Error('Missing compliance monitoring assessment id');
  }
