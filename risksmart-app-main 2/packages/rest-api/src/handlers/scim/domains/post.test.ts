...
describe('SCIM Add Domain Proxy Handler', () => {
  beforeEach(() => {
  ...
  it('should successfully proxy the request and return 200 with domain details', async () => {
    const response = await handler(
      mockEvent({ domain: 'example.com' }),
      {} as Context
    );
  ...
  test.each([
    [{ invalidField: 'wrong' } as unknown as PostSchema, 'invalid_type'],
    [{ domain: '' } as unknown as PostSchema, 'too_small'],
    [{ domain: 123 } as unknown as PostSchema, 'invalid_type'],
  ])(
  ...
  it('should sign the request properly before calling the API', async () => {
    const signRequestSpy = vi.spyOn(
      await import('src/signRequest'),
      'signRequest'
    );

    await handler(mockEvent({ domain: 'example.com' }), {} as Context);

    expect(signRequestSpy).toHaveBeenCalledWith(
      expect.stringContaining(`organisation/${mockOrgKey}/domains`),
      'POST',
      {},
      JSON.stringify({ tenant: 'test-tenant', domain: 'example.com' })
    );
  });
