...
export const handler = backendRouteHandler(deleteSchema, async (body) => {
  try {
    ...
    const keyId = body.input.keyId;

    const scimApiUrl = getEnv('SCIM_INTERNAL_API_URL');
    logger.info('Deleting SCIM token for organisation', {
      orgKey,
      scimApiUrl,
      keyId,
    });

    const url = `${scimApiUrl}/organisation/${orgKey}/tokens/${keyId}`;

    const bodyData = JSON.stringify({ tenant });
    const headers = await signRequest(url, 'DELETE', {}, bodyData);

    logger.info('Calling internal API', { url, tenant });
