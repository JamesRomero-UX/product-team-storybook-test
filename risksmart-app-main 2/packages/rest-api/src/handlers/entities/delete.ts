...
export const handler = backendRouteHandler(DeleteSchema, async (request) => {
  ...
  await checkPermission(
    request,
    ParentTypeEnum.Entity,
    AccessTypeEnum.Delete,
    request.input.Id
  );

  const result = await apiClient.deleteEntity({
    where: { Id: { _eq: request.input.Id } },
  });

  const affectedRows = result.delete_entity?.affected_rows;
  if (!affectedRows) {
    ...
    return {
      statusCode: 404,
      body: JSON.stringify({
        affected_rows: result.delete_entity?.affected_rows,
      }),
    };
  }
