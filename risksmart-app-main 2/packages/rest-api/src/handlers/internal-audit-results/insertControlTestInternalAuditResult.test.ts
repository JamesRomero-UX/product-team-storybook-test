...
import { handler } from './insertControlTestInternalAuditResult';

vi.mock('src/backendGraphqlClient');
vi.mock('src/services/node/node.service');
vi.mock('src/services/role-access/roleAccessService');
vi.mock(
  'src/services/internal-audit-test-result/internalAuditTestResultService'
);
...
const insertTestResultWithParentsMock = vi.mocked(
  insertInternalAuditTestResultWithParents
);
...
describe('insert control test internal audit result - post', () => {
  beforeEach(() => {
    vi.resetAllMocks();
