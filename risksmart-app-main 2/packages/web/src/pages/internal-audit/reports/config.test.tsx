...
import type { InternalAuditReportFields } from './types';

describe('internal audit reports config', () => {
  const defaultAssessment: InternalAuditReportFields = {
  ...
  const TestHarness: FC<{ records: InternalAuditReportFields[] }> = ({
    records,
  }) => {
  ...
  const testMocks = [
    ...defaultMocks,
    mockedGetFormCustomisationResponse([Parent_Type_Enum.InternalAuditReport]),
    mockedGetAggregationResponse(),
    mockedGetUserTablePreferences('internalAuditReportRegister'),
  ];
  ...
  describe('useGetCollectionTableProps', () => {
    const nextAssessmentDate = 'Next audit date';
    ...
    it('should display friendly sequential id for the ID column', async () => {
      ...
      expect(getCellText(container, id, 1)).toEqual('IAR-1');
    });

    it('should display the "Next assessment date" when toggled on in preferences', async () => {
      const { container } = render(
        <TestHarness records={[{ ...defaultAssessment }]} />,
        {
          wrapper: getWrapper(testMocks, ...providers),
    ...
    it('should display correct empty collection text', async () => {
      ...
      await waitForTableHeaders(container);
      expect(getEmptyCollectionSlotText(container, 0)).toEqual('No Reports');
