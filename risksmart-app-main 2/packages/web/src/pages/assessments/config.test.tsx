...
import type { AssessmentFields } from './types';

describe('assessments config', () => {
  const defaultAssessment: AssessmentFields = {
  ...
  const TestHarness: FC<{ records: AssessmentFields[] }> = ({ records }) => {
  ...
  const testMocks = [
    ...defaultMocks,
    mockedGetFormCustomisationResponse([Parent_Type_Enum.Assessment]),
    mockedGetAggregationResponse(),
    mockedGetUserTablePreferences('assessmentRegister'),
  ];
  ...
  describe('useGetCollectionTableProps', () => {
    ...
    it('should display friendly sequential id for the ID column', async () => {
      ...
      expect(getCellText(container, id, 1)).toEqual('ASMT-1');
    });
