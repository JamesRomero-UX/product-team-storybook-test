...
/**
 * Calculate the initial schedule state for a newly created entity.
 * For new entities there are no test results yet, so latestDate is always null.
 */
export function calculateInitialScheduleState(schedule: {
  Frequency?: TestFrequency | null;
  StartDate?: string | null;
  ManualDueDate?: string | null;
  TimeToCompleteValue?: number | null;
  TimeToCompleteUnit?: UnitOfTime | null;
}): { DueDate: string | null; OverdueDate: string | null; LatestDate: null } {
  const dueDate =
    schedule.Frequency === TestFrequency.Adhoc
      ? (schedule.ManualDueDate ?? null)
      : getDueDate({
  ...
  return { DueDate: dueDate, OverdueDate: overdueDate, LatestDate: null };
}
