import { buildIndicator, insertIndicators } from '@risksmart-app/test-data';
import {
  createTestContext,
  type TestContext,
...
describe('GET /api/v1/indicators', () => {
  ...
  it('should return a list of indicators with valid token', async () => {
    const { httpClient } = context;

    const response = await httpClient.get<IndicatorListResponse>('/indicators');
  ...
  it('should get indicator item returned in list from the api', async () => {
    const { httpClient } = context;

    const response = await httpClient.get<IndicatorListResponse>('/indicators');
    expect(response.data.data.length).toBeGreaterThan(0);

    const indicatorItemId = response.data.data[0]!.id;
    const itemResponse = await httpClient.get<IndicatorListItem>(
      `/indicators/${indicatorItemId}`
    );
  ...
  it('should support pagination with limit parameter', async () => {
    ...
    const response = await httpClient.get<IndicatorListResponse>(
      '/indicators',
      {
        params: { page_size: 2 },
      }
    ...
    const nextPageResponse = await httpClient.get<IndicatorListResponse>(
  ...
  it('should return 401 without token', async () => {
    const response = await fetch(
      `${process.env.EXTERNAL_API_URL}/api/v1/indicators`,
      {
        method: 'GET',
        headers: {
  ...
  it('should return 403 with insufficient scope', async () => {
    ...
    const response =
      await limitedContext.httpClient.get<IndicatorListResponse>('/indicators');
