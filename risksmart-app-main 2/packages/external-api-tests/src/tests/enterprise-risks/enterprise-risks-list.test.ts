...
interface EnterpriseRiskListItem {
  id: string;
  title: string;
}

interface EnterpriseRiskListResponse {
  data: EnterpriseRiskListItem[];
  pageInfo: {
    count: number;
    nextPage: string;
    prevPage: string;
...
describe('GET /api/v1/enterprise-risks', () => {
  ...
  beforeAll(async () => {
    ...
    const enterpriseRisks = Array.from({ length: 4 }, (_, i) =>
      buildEnterpriseRisk({
        orgKey,
        userId,
        overrides: {
          Title: `Test Enterprise Risk ${i + 1}`,
        },
      })
    );

    await insertEnterpriseRisks(enterpriseRisks);
    await waitForDbPropagation();
  });

  it('should return a list of enterprise risks with valid token', async () => {
    const { httpClient } = context;

    const response =
      await httpClient.get<EnterpriseRiskListResponse>('/enterprise-risks');
  ...
  it('should get enterprise risk item returned in list from the api', async () => {
    const { httpClient } = context;
    ...
    const itemResponse = await httpClient.get<EnterpriseRiskListItem>(
      `/enterprise-risks/${itemId}`
    );
  ...
  it('should support pagination with limit parameter', async () => {
    ...
    const response = await httpClient.get<EnterpriseRiskListResponse>(
      '/enterprise-risks',
      {
        params: { page_size: 2 },
      }
    ...
    const nextPageResponse = await httpClient.get<EnterpriseRiskListResponse>(
  ...
  it('should return 401 without token', async () => {
    const response = await fetch(
      `${process.env.EXTERNAL_API_URL}/api/v1/enterprise-risks`,
      {
        method: 'GET',
        headers: {
  ...
  it('should return 403 with insufficient scope', async () => {
    ...
    const response =
      await limitedContext.httpClient.get<EnterpriseRiskListResponse>(
        '/enterprise-risks'
      );
