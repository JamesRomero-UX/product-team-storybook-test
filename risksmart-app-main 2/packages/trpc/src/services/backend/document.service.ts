...
import type { DocumentListResponse } from '../../types/backend/v1/list.types';
import {
  computePageAndMeta,
...
export class DocumentServiceImpl implements DocumentBackendService {
  async getDocumentList(
    ...
    return { pageMetadata: metadata, document: page };
  }
  async getDocumentById(ctx: BackendServiceContext, id: string) {
    const db = await createDrizzleClient(ctx);

    return db.org(async (tx) => {
      const [formConfig, documentData] = await Promise.all([
      ...
      return documentData
        ? { document: documentData, form_configuration: formConfig ?? null }
        : null;
    });
  }
}
