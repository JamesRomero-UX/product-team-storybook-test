...
import {
  getObligationSecondLineResults,
...
import { buildObligationSecondLineResult } from '../data/secondLineResult';
...
describe('obligationSecondLineResult', () => {
  beforeEach(async () => {
  ...
  describe('query', () => {
    it.each([
      ...
      async ({ expectedRecords, ...user }) => {
        const assessment = buildComplianceMonitoringAssessment();
        ...
        await insertObligationSecondLineResult(
          buildObligationSecondLineResult({
            parents: {
              data: [
                {
        ...
        const assessments = await getObligationSecondLineResults(
          {
            ParentId: assessment.Id!,
          },
          {
              ...
              data: [
                {
                  ...
                  ResultType: ParentTypeEnum.ObligationSecondLineResult,
                  OrgKey: getDefaultOrgId(),
                  CreatedByUser: getDefaultUserId(),
                  CreatedAtTimestamp: new Date().toISOString(),
                  ModifiedByUser: getDefaultUserId(),
  ...
  describe('insert', () => {
    it.each([{ ...riskManagerUser1 }])(
      ...
      async ({ ...user }) => {
        ...
        const result = await insertChildObligationSecondLineResult(
          {
            ...buildObligationSecondLineResult({
              Id: undefined,
              parents: undefined,
    ...
    it.each([
      ...
      },
      {
        ...readOnlyUser1,
        expected:
          "field 'insertChildObligationSecondLineResult' not found in type: 'mutation_root'",
      },
    ])(
