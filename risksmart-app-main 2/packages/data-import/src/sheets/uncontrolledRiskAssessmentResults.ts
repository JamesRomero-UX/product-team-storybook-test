...
export const generateMockData = (): CsvType[] => {
  ...
  for (
    // Fudge to avoid id collisions with controlled risk assessment results
    let i = generateConfig.assessmentsCount;
    i < generateConfig.assessmentsCount * 2;
    i++
  ) {
    const id = i + 1;
...
const sheet: Sheet<
  ...
  InsertType
> = {
  name: 'uncontrolledRiskAssessmentResults.csv',
  schema,
  objectType: ParentTypeEnum.RiskAssessmentResult,
  customAttributeType: ParentTypeEnum.UncontrolledRiskAssessmentResult,
  fields: [
    {
      key: 'id',
      type: 'string',
