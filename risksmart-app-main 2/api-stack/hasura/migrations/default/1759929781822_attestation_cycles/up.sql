-- Table definitions
CREATE TABLE IF NOT EXISTS risksmart.attestation_cycle (
    "Id" uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    "Status" text NOT NULL,
    "AllowCarryForward" boolean NOT NULL DEFAULT false,
    "ParentId" uuid NOT NULL REFERENCES risksmart.document_file("Id") ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS "idx_attestation_cycle_orgkey" on risksmart.attestation_cycle("OrgKey");

CREATE TABLE IF NOT EXISTS risksmart.attestation_cycle_audit (
    "Id" uuid NOT NULL,
    "Status" text NOT NULL,
    ...
    CONSTRAINT attestation_cycle_audit_pkey PRIMARY KEY ("Id", "OrgKey", "ModifiedAtTimestamp")
);

CREATE OR REPLACE FUNCTION risksmart.attestation_cycle_modified() RETURNS trigger AS $body$
DECLARE nr RECORD;
...
INSERT INTO risksmart."attestation_cycle_audit" (
        "Id",
        "Status",
        "AllowCarryForward",
        "ParentId",
...
ALTER TABLE risksmart.attestation_cycle ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.attestation_cycle_audit ENABLE ROW LEVEL SECURITY;

CREATE TRIGGER attestation_cycle_audit_trigger
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON risksmart.attestation_cycle FOR EACH ROW EXECUTE FUNCTION risksmart.attestation_cycle_modified();

CREATE POLICY own_org ON risksmart.attestation_cycle_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.attestation_cycle TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.attestation_cycle_audit TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.attestation_cycle TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
...
-- Update attestation record to optionally link to attestation cycle
-- Add optional foreign key to existing attestation_record table
ALTER TABLE risksmart.attestation_record
ADD COLUMN "CycleId" UUID REFERENCES risksmart.attestation_cycle("Id") ON DELETE CASCADE;

ALTER TABLE risksmart.attestation_record_audit
ADD COLUMN "CycleId" uuid;

-- Create partial index for performance (only index non-null values)
CREATE INDEX idx_attestation_record_cycle_id ON risksmart.attestation_record("CycleId")
WHERE "CycleId" IS NOT NULL;

CREATE OR REPLACE FUNCTION risksmart.attestation_record_modified() RETURNS trigger LANGUAGE plpgsql AS $function$
DECLARE nr RECORD;
