/* Create risk_assessment_result_impact table */
CREATE TABLE IF NOT EXISTS risksmart.risk_assessment_result_impact (
    "Id" UUID DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    "RiskAssessmentResultId" UUID NOT NULL REFERENCES risksmart.risk_assessment_result("Id") ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS "idx_risk_assessment_result_impact_orgkey"
    ON risksmart.risk_assessment_result_impact("OrgKey");

CREATE UNIQUE INDEX IF NOT EXISTS "idx_risk_assessment_result_impact_parent_label"
    ON risksmart.risk_assessment_result_impact("RiskAssessmentResultId", "Label");

/* Create node type for risk_assessment_result_impact */
INSERT INTO risksmart.node_type ("Value", "Comment")
VALUES ('risk_assessment_result_impact', 'Risk Assessment Result Impact') ON CONFLICT DO NOTHING;

/* Create risk_assessment_result_impact_audit table */
CREATE TABLE IF NOT EXISTS risksmart.risk_assessment_result_impact_audit (
    "Id" UUID NOT NULL,
    "RiskAssessmentResultId" UUID NOT NULL,
    ...
    CONSTRAINT risk_assessment_result_impact_audit_pkey PRIMARY KEY ("Id", "OrgKey", "ModifiedAtTimestamp")
);

/* Create function to insert audit record when risk_assessment_result_impact is modified */
CREATE OR REPLACE FUNCTION risksmart.risk_assessment_result_impact_modified()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $func$
...
BEGIN
    ...
    INSERT INTO risksmart.risk_assessment_result_impact_audit (
        "Id",
...
CREATE OR REPLACE TRIGGER risk_assessment_result_impact_audit_trigger
AFTER INSERT OR UPDATE OR DELETE
ON risksmart.risk_assessment_result_impact
FOR EACH ROW
EXECUTE FUNCTION risksmart.risk_assessment_result_impact_modified();

/* Add row level security */
ALTER TABLE risksmart.risk_assessment_result_impact ENABLE ROW LEVEL SECURITY;
ALTER TABLE risksmart.risk_assessment_result_impact_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY own_org ON risksmart.risk_assessment_result_impact TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.risk_assessment_result_impact_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.risk_assessment_result_impact TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.risk_assessment_result_impact_audit TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
