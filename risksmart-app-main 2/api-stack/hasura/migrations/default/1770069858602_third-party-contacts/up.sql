-- Third Party Contact table for managing contacts separately from questionnaire invites
CREATE TABLE IF NOT EXISTS risksmart.third_party_contact (
  "Id" uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  "ThirdPartyId" uuid NOT NULL REFERENCES risksmart.third_party("Id") ON DELETE CASCADE,
  "Email" text NOT NULL,
  "Name" text,
  "JobTitle" text,
  ...
  -- Unique constraint: one contact email per third party
  CONSTRAINT "third_party_contact_email_unique" UNIQUE ("ThirdPartyId", "Email")
);

-- Index for faster lookups by third party
CREATE INDEX IF NOT EXISTS idx_third_party_contact_third_party_id ON risksmart.third_party_contact("ThirdPartyId");

-- Index for OrgKey filtering
CREATE INDEX IF NOT EXISTS idx_third_party_contact_orgkey ON risksmart.third_party_contact("OrgKey");

-- Index for UserId lookups
CREATE INDEX IF NOT EXISTS idx_third_party_contact_userid ON risksmart.third_party_contact("UserId");

-- Audit table
CREATE TABLE IF NOT EXISTS risksmart.third_party_contact_audit (LIKE risksmart.third_party_contact);

DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
  WHERE conname = 'third_party_contact_audit_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
  ...
  WHERE table_schema = 'risksmart'
    AND table_name = 'third_party_contact_audit'
...
ALTER TABLE risksmart.third_party_contact ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.third_party_contact_audit ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS own_org ON risksmart.third_party_contact;

CREATE POLICY own_org ON risksmart.third_party_contact TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

DROP POLICY IF EXISTS own_org ON risksmart.third_party_contact_audit;

CREATE POLICY own_org ON risksmart.third_party_contact_audit TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

DROP POLICY IF EXISTS own_org_rw ON risksmart.third_party_contact;

CREATE POLICY own_org_rw ON risksmart.third_party_contact TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

DROP POLICY IF EXISTS own_org_rw ON risksmart.third_party_contact_audit;

CREATE POLICY own_org_rw ON risksmart.third_party_contact_audit TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

DROP POLICY IF EXISTS own_org_data_layer ON risksmart.third_party_contact;

CREATE POLICY own_org_data_layer ON risksmart.third_party_contact FOR ALL TO data_layer USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

DROP POLICY IF EXISTS own_org_data_layer ON risksmart.third_party_contact_audit;

CREATE POLICY own_org_data_layer ON risksmart.third_party_contact_audit FOR ALL TO data_layer USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);
...
-- Audit trigger
DROP TRIGGER IF EXISTS third_party_contact_audit_trigger ON risksmart.third_party_contact;

CREATE TRIGGER third_party_contact_audit_trigger
AFTER
INSERT
  OR DELETE
  OR
UPDATE ON risksmart.third_party_contact FOR EACH ROW EXECUTE FUNCTION risksmart.third_party_contact_modified();

-- Add ContactId foreign key to questionnaire_invite for linking invites to contacts
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM information_schema.columns
  WHERE table_schema = 'risksmart'
    AND table_name = 'questionnaire_invite'
    AND column_name = 'ContactId'
) THEN
ALTER TABLE risksmart.questionnaire_invite
ADD COLUMN "ContactId" uuid REFERENCES risksmart.third_party_contact("Id");
...
CREATE INDEX IF NOT EXISTS idx_questionnaire_invite_contact_id ON risksmart.questionnaire_invite("ContactId");
