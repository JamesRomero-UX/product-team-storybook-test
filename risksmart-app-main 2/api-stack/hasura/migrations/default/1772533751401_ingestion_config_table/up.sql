CREATE TABLE IF NOT EXISTS risksmart.ingestion_config (
  ...
  CONSTRAINT "ingestion_config_orgKey_fkey" FOREIGN KEY ("OrgKey") REFERENCES auth.organisation ("OrgKey"),
  CONSTRAINT "ingestion_config_createdByUser_fkey" FOREIGN KEY ("CreatedByUser") REFERENCES auth."user" ("Id"),
  CONSTRAINT "ingestion_config_modifiedByUser_fkey" FOREIGN KEY ("ModifiedByUser") REFERENCES auth."user" ("Id")
);

CREATE TABLE IF NOT EXISTS risksmart.ingestion_config_audit (
  "Id" uuid NOT NULL,
  "OrgKey" text NOT NULL,
  "IngestionConfig" jsonb,
  "CreatedByUser" text NOT NULL,
...
CREATE TRIGGER ingestion_config_audit_trigger
AFTER
INSERT
  OR DELETE
  OR
UPDATE ON risksmart.ingestion_config FOR EACH ROW EXECUTE FUNCTION risksmart.ingestion_config_modified();

CREATE INDEX "idx_ingestion_config_orgkey" ON risksmart.ingestion_config("OrgKey");

ALTER TABLE risksmart.ingestion_config ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.ingestion_config_audit ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY own_org ON risksmart.ingestion_config_audit TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.ingestion_config_audit FOR ALL TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.ingestion_config_audit FOR ALL TO data_layer USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.ingestion_config TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.ingestion_config FOR ALL TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.ingestion_config FOR ALL TO data_layer USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);