CREATE TABLE risksmart."obligation_change" (
    "Id" UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    "SequentialId" INTEGER NOT NULL,
    "ObligationId" UUID REFERENCES risksmart."obligation" ("Id") ON DELETE CASCADE,
...
CREATE TABLE risksmart."obligation_change_audit" (
    "Id" UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    "SequentialId" INTEGER NOT NULL,
    "ObligationId" UUID,
    "ExternalId" TEXT NOT NULL,
...
CREATE TRIGGER obligation_change_audit_trigger
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON risksmart.obligation_change FOR EACH ROW EXECUTE FUNCTION risksmart.obligation_change_modified();

CREATE TRIGGER a_set_sequential_id_trigger BEFORE
INSERT ON risksmart.obligation_change for each ROW EXECUTE PROCEDURE risksmart.set_sequential_id();

CREATE TRIGGER node_insert_trigger before
insert ON risksmart.obligation_change FOR EACH ROW EXECUTE PROCEDURE risksmart.node_insert();

CREATE TRIGGER node_delete_trigger
AFTER DELETE ON risksmart.obligation_change FOR EACH ROW EXECUTE PROCEDURE risksmart.node_delete();

ALTER TABLE risksmart.obligation_change ENABLE ROW LEVEL SECURITY;

CREATE POLICY own_org ON risksmart.obligation_change TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE TABLE risksmart."obligation_change_attestation" (
    "Id" UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    "ObligationChangeId" UUID REFERENCES risksmart."obligation_change"("Id") ON DELETE CASCADE,
...
CREATE TABLE risksmart."obligation_change_attestation_audit" (
    "Id" UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE OR REPLACE FUNCTION risksmart.obligation_change_attestation_modified() RETURNS trigger AS $body$
DECLARE nr RECORD;
...
insert into risksmart.obligation_change_attestation_audit(
        "Id",
        "ObligationChangeId",
        "UserId",
        "CreatedByUser",
...
CREATE TRIGGER obligation_change_attestation_audit_trigger
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON risksmart.obligation_change_attestation FOR EACH ROW EXECUTE FUNCTION risksmart.obligation_change_attestation_modified();

CREATE TRIGGER node_insert_trigger before
insert ON risksmart.obligation_change_attestation FOR EACH ROW EXECUTE PROCEDURE risksmart.node_insert();

CREATE TRIGGER node_delete_trigger
AFTER DELETE ON risksmart.obligation_change_attestation FOR EACH ROW EXECUTE PROCEDURE risksmart.node_delete();

ALTER TABLE risksmart.obligation_change_attestation ENABLE ROW LEVEL SECURITY;

CREATE POLICY own_org ON risksmart.obligation_change_attestation TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
...
VALUES (
        'obligation_change',
        'Obligation change feed items'
    ),
    (
        'obligation_change_attestation',
        'Attestations for obligation change feed items'
    );
...
CREATE INDEX "idx_obligation_change_orgkey" on risksmart.obligation_change("OrgKey");

CREATE INDEX "idx_obligation_change_attestation_orgkey" on risksmart.obligation_change_attestation("OrgKey");

CREATE POLICY own_org_rw ON risksmart.obligation_change FOR ALL TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.obligation_change FOR ALL TO data_layer USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.obligation_change_attestation FOR ALL TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.obligation_change_attestation FOR ALL TO data_layer USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

ALTER TABLE risksmart.obligation_change_attestation_audit ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.obligation_change_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY own_org ON risksmart.obligation_change_attestation_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.obligation_change_attestation_audit FOR ALL TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.obligation_change_attestation_audit FOR ALL TO data_layer USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.obligation_change_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.obligation_change_audit FOR ALL TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_data_layer ON risksmart.obligation_change_audit FOR ALL TO data_layer USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
...
UNION ALL
SELECT NULL::text AS "Item",
    'data_export_schedule_execution'::text AS "ObjectType",
    data_export_schedule_execution_audit."ParentId"::text AS "Id",
    data_export_schedule_execution_audit."Action",
    data_export_schedule_execution_audit."ModifiedByUser",
    data_export_schedule_execution_audit."ModifiedAtTimestamp",
    data_export_schedule_execution_audit."OrgKey"
FROM risksmart.data_export_schedule_execution_audit
UNION ALL
SELECT NULL::text AS "Item",
...
UNION ALL
SELECT NULL::text AS "Item",
    'obligation_change'::text AS "ObjectType",
    obligation_change_audit."Id"::text AS "Id",
    obligation_change_audit."Action",
    obligation_change_audit."ModifiedByUser",
    obligation_change_audit."ModifiedAtTimestamp",
    obligation_change_audit."OrgKey"
FROM risksmart.obligation_change_audit
UNION ALL
SELECT NULL::text AS "Item",
    'obligation_change_attestation'::text AS "ObjectType",
    obligation_change_attestation_audit."Id"::text AS "Id",
    obligation_change_attestation_audit."Action",
    obligation_change_attestation_audit."ModifiedByUser",
    obligation_change_attestation_audit."ModifiedAtTimestamp",
    obligation_change_attestation_audit."OrgKey"
FROM risksmart.obligation_change_attestation_audit;
