...
VALUES ('colour_palette', 'Colour Palette') ON CONFLICT DO NOTHING;
...
-- Add colour palette tab to settings
UPDATE risksmart."tab"
SET "Tabs" = jsonb_set(
    "Tabs"::jsonb,
    '{default}',
    "Tabs"->'default' || '{"id": "colours"}',
    true
  )
WHERE "ParentType" IN ('settings');

--Create palette tables and triggers
CREATE TABLE risksmart."colour_palette" (
...
CREATE TABLE risksmart."colour_palette_audit" (
  "Id" uuid NOT NULL,
  ...
  CONSTRAINT colour_palette_audit_pkey PRIMARY KEY ("Id", "OrgKey", "ModifiedAtTimestamp")
);

CREATE INDEX IF NOT EXISTS "idx_colour_palette_orgkey" on risksmart.colour_palette("OrgKey");

CREATE OR REPLACE FUNCTION risksmart.colour_palette_modified() RETURNS trigger AS $body$
DECLARE nr RECORD;
...
INSERT INTO risksmart."colour_palette_audit" (
    "Id",
    "OrgKey",
    "Name",
    "Settings",
...
ALTER TABLE risksmart.colour_palette_audit ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.colour_palette ENABLE ROW LEVEL SECURITY;

CREATE TRIGGER colour_palette_audit_trigger
AFTER
INSERT
  OR DELETE
  OR
UPDATE ON risksmart.colour_palette FOR EACH ROW EXECUTE FUNCTION risksmart.colour_palette_modified();

CREATE POLICY own_org ON risksmart.colour_palette_audit TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.colour_palette TO reporting USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.colour_palette_audit TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.colour_palette TO trpc USING (
  "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
  "OrgKey" = current_setting('risksmart.org_key', 't')
);
