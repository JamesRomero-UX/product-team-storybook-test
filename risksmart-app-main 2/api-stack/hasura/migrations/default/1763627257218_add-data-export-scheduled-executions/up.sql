-- Table definitions
CREATE TABLE IF NOT EXISTS risksmart.data_export_schedule_execution_status (
  "Value" text NOT NULL PRIMARY KEY,
  "Comment" text
);


INSERT INTO risksmart.data_export_schedule_execution_status ("Value", "Comment")
VALUES ('scheduled', 'Scheduled'),
       ('complete', 'Complete'),
       ('failed', 'Failed'),
       ('cancelled', 'Cancelled');

CREATE TABLE IF NOT EXISTS risksmart.data_export_schedule_execution (
    "ExecutionTimestamp" TIMESTAMPTZ NOT NULL,
    "Status" text NOT NULL REFERENCES risksmart.data_export_schedule_execution_status("Value") ON DELETE CASCADE,
    "Errors" text,
    "ParentId" uuid NOT NULL REFERENCES risksmart.data_export_schedule("Id") ON DELETE CASCADE,
    ...
    CONSTRAINT data_export_schedule_execution_pkey PRIMARY KEY ("ParentId", "ExecutionTimestamp")
);

-- Trigger to insert audit record on changes
CREATE INDEX IF NOT EXISTS "idx_data_export_schedule_execution_orgkey" on risksmart.data_export_schedule_execution("OrgKey");

CREATE TABLE IF NOT EXISTS risksmart.data_export_schedule_execution_audit (
    "ExecutionTimestamp" TIMESTAMPTZ NOT NULL,
    ...
    CONSTRAINT data_export_schedule_execution_audit_pkey PRIMARY KEY ("ParentId", "ExecutionTimestamp", "OrgKey", "ModifiedAtTimestamp")
);

CREATE OR REPLACE FUNCTION risksmart.data_export_schedule_execution_modified() RETURNS trigger AS $body$
DECLARE nr RECORD;
...
INSERT INTO risksmart."data_export_schedule_execution_audit" (
        "ExecutionTimestamp",
        "Status",
        "Errors",
        "ParentId",
...
ALTER TABLE risksmart.data_export_schedule_execution ENABLE ROW LEVEL SECURITY;

ALTER TABLE risksmart.data_export_schedule_execution_audit ENABLE ROW LEVEL SECURITY;

CREATE TRIGGER data_export_schedule_execution_audit_trigger
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON risksmart.data_export_schedule_execution FOR EACH ROW EXECUTE FUNCTION risksmart.data_export_schedule_execution_modified();

CREATE POLICY own_org ON risksmart.data_export_schedule_execution_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.data_export_schedule_execution TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.data_export_schedule_execution_audit TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.data_export_schedule_execution TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
