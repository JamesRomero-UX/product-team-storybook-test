/* Create risk_assessment_result_config table */
CREATE TABLE IF NOT EXISTS risksmart.risk_assessment_result_config (
...
CREATE INDEX IF NOT EXISTS "idx_risk_assessment_result_config_orgkey"
    ON risksmart.risk_assessment_result_config("OrgKey");

CREATE UNIQUE INDEX IF NOT EXISTS "idx_risk_assessment_result_config_orgkey_version"
    ON risksmart.risk_assessment_result_config("OrgKey", "Version");

CREATE UNIQUE INDEX IF NOT EXISTS "idx_risk_assessment_result_config_orgkey_islatest"
    ON risksmart.risk_assessment_result_config("OrgKey") WHERE "IsLatest" = TRUE;

/* Create node type for risk assessment result config */
INSERT INTO risksmart."node_type" ("Value", "Comment")
VALUES ('risk_assessment_result_config', 'Risk Assessment Result Config') ON CONFLICT DO NOTHING;

/* Create risk_assessment_result_config_audit table */
CREATE TABLE IF NOT EXISTS risksmart.risk_assessment_result_config_audit (
    "Id" UUID NOT NULL,
    ...
    CONSTRAINT risk_assessment_result_config_audit_pkey PRIMARY KEY ("Id", "OrgKey", "ModifiedAtTimestamp")
);

/* Create function to insert audit record when risk_assessment_result_config is modified */
CREATE OR REPLACE FUNCTION risksmart.risk_assessment_result_config_modified()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $func$

DECLARE record RECORD;
DECLARE modified_by_user TEXT;
DECLARE modified_at_timestamp TIMESTAMPTZ;

BEGIN
    IF (TG_OP = 'UPDATE' OR TG_OP = 'INSERT') THEN
        record := NEW;
        modified_by_user := NEW."ModifiedByUser";
        modified_at_timestamp := NEW."ModifiedAtTimestamp";
    ELSIF (TG_OP = 'DELETE') THEN
        record := OLD;
        modified_by_user := risksmart.get_hasura_user_id();
        modified_at_timestamp := statement_timestamp();
    END IF;

    INSERT INTO risksmart.risk_assessment_result_config_audit (
        "Id",
        "OrgKey",
        "Version",
        "Config",
...
/* Create trigger to maintain audit table */
CREATE OR REPLACE TRIGGER risk_assessment_result_config_audit_trigger
AFTER INSERT OR UPDATE OR DELETE
ON risksmart.risk_assessment_result_config
FOR EACH ROW
EXECUTE FUNCTION risksmart.risk_assessment_result_config_modified();

/* Create function to auto-increment Version and set IsLatest = FALSE on previous versions */
CREATE OR REPLACE FUNCTION risksmart.risk_assessment_result_config_set_latest()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $func$
BEGIN
    /* Auto-calculate the next version number for this org */
    NEW."Version" := COALESCE(
        (SELECT MAX("Version") + 1 
         FROM risksmart.risk_assessment_result_config 
         WHERE "OrgKey" = NEW."OrgKey"), 
        1
    );

    /* Set IsLatest = FALSE on any existing records for this org */
    UPDATE risksmart.risk_assessment_result_config
    SET "IsLatest" = FALSE,
...
/* Create trigger to maintain IsLatest flag */
CREATE OR REPLACE TRIGGER risk_assessment_result_config_set_latest_trigger
BEFORE INSERT
ON risksmart.risk_assessment_result_config
FOR EACH ROW
EXECUTE FUNCTION risksmart.risk_assessment_result_config_set_latest();

/* Add row level security */
ALTER TABLE risksmart.risk_assessment_result_config ENABLE ROW LEVEL SECURITY;
ALTER TABLE risksmart.risk_assessment_result_config_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY own_org ON risksmart.risk_assessment_result_config TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org ON risksmart.risk_assessment_result_config_audit TO reporting USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.risk_assessment_result_config TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);

CREATE POLICY own_org_rw ON risksmart.risk_assessment_result_config_audit TO trpc USING (
    "OrgKey" = current_setting('risksmart.org_key', 't')
) WITH CHECK (
    "OrgKey" = current_setting('risksmart.org_key', 't')
);
