...
function v() {
  const t = a.resolve(process.cwd(), "package.json"), e = JSON.parse(l.readFileSync(t, "utf8")), s = d(e, process.cwd()), n = y(), r = (c) => {
    ...
    for (const u of Object.values(c)) {
      ...
      try {
        const g = JSON.parse(l.readFileSync(o, "utf8")), h = d(g, p);
        if (Object.assign(i, h), h) {
          const k = r(h);
          Object.assign(i, k);
        }
      } catch {
...
const j = async ({ root: t, config: e }) => {
  const s = v(), n = e.components === !0 ? Object.values(s).filter(
    ...
    (g) => f({ path: g.path, fileName: `${o}.css` })
