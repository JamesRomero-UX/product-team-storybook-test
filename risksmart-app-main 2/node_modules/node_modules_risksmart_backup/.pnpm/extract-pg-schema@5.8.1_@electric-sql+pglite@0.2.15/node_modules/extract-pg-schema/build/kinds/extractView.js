...
const commentMapQueryPart_1 = __importDefault(require("./query-parts/commentMapQueryPart"));
...
`;
const resolveSource = (column, sourceMapping) => ({
    ...column,
...
const extractView = async (db, view) => {
    ...
    const triggersQuery = await db("information_schema.triggers")
        .where({
    ...
    for (const [triggerName, triggerRows] of Object.entries(triggersByName)) {
        ...
        triggers.push({
            name: triggerName,
            eventManipulation: triggerRows.map((r) => r.event_manipulation),
            actionTiming: first.action_timing,
  ...
  `, { table_name: view.name, schema_name: view.schemaName });
    const unresolvedColumns = columnsQuery.rows;
    let sourceMapping;
    try {
        const viewReferences = await (0, parseViewDefinition_1.default)(informationSchemaValue.view_definition, view.schemaName);
        sourceMapping = R.indexBy(R.prop("viewColumn"), viewReferences);
    }
    catch {
        console.warn(`Error parsing view definition for "${view.name}". Falling back to raw data`);
    }
    const columns = unresolvedColumns.map((column) => resolveSource(column, sourceMapping));
    return {
        ...view,
