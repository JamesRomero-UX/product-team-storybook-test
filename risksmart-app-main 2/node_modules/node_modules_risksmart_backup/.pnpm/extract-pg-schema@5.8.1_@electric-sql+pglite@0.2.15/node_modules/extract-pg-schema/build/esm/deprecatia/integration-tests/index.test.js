...
describe("extractSchema", () => {
    ...
    beforeAll(async () => {
        ...
        const genericContainer = new GenericContainer("kristiandupont/dvdrental-image")
    ...
    test("in first schema", async () => {
        ...
        expect(extracted.types.filter((t) => t.name === "cust_type_not_default")).toHaveLength(0);
    });
    test("in secondary schema", async () => {
        ...
        expect(extracted.types.filter((t) => t.name === "cust_type_not_default")).toHaveLength(1);
    });
...
`);
 ...
 ;`);
            await db.destroy();
            const extracted = await extractSchema("partition_test", connection, false);
            expect(extracted.tables).toHaveLength(1);
        });
    });
