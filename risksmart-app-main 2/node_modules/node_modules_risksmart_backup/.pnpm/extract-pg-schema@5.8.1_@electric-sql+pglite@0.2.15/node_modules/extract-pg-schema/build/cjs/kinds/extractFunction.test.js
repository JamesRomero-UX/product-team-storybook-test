...
const extractFunction_1 = __importDefault(require("./extractFunction"));
const makePgType = (name, schemaName = "test") => ({
    schemaName,
    name,
    kind: "function",
    comment: null,
});
(0, vitest_1.describe)("extractFunction", () => {
    const [getKnex] = (0, useTestKnex_1.default)();
    (0, useSchema_1.default)(getKnex, "test");
    (0, vitest_1.it)("should extract function details", async () => {
        const db = getKnex();
        await db.raw("create function test.some_function() returns text as $$ BEGIN return 'hello'; END; $$ language plpgsql");
        const result = await (0, extractFunction_1.default)(db, makePgType("some_function"));
        const expected = {
            schemaName: "test",
            name: "some_function",
            kind: "function",
        ...
        (0, vitest_1.expect)(result[0]).toMatchObject(expected);
    });
    (0, vitest_1.it)("should extract function details with arguments", async () => {
        const db = getKnex();
        await db.raw("create function test.some_function(text) returns text as $$ BEGIN return $1; END; $$ language plpgsql");
        const result = await (0, extractFunction_1.default)(db, makePgType("some_function"));
        const expected = {
            schemaName: "test",
            name: "some_function",
    ...
    (0, vitest_1.it)("should handle different parameter modes", async () => {
        const db = getKnex();
        await db.raw(`
      ...
      END;
      $$ language plpgsql`);
        const result = await (0, extractFunction_1.default)(db, makePgType("param_modes"));
        (0, vitest_1.expect)(result[0].parameters).toMatchObject([
    ...
    (0, vitest_1.it)("should handle complex return types", async () => {
        const db = getKnex();
        await db.raw(`
      ...
      END;
      $$ language plpgsql`);
        const result = await (0, extractFunction_1.default)(db, makePgType("returns_complex"));
        (0, vitest_1.expect)(result[0].returnType).toMatchObject({
            type: "table",
            columns: [
    ...
    (0, vitest_1.it)("should handle function attributes", async () => {
        const db = getKnex();
        await db.raw(`
      ...
      END;
      $$`);
        const result = await (0, extractFunction_1.default)(db, makePgType("with_attributes"));
