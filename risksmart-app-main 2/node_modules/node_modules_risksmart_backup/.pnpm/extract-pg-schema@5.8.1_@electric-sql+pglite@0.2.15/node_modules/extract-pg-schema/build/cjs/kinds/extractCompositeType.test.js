...
const extractCompositeType_1 = __importDefault(require("./extractCompositeType"));
...
(0, vitest_1.describe)("extractCompositeType", () => {
    const [getKnex, databaseName] = (0, useTestKnex_1.default)();
    (0, useSchema_1.default)(getKnex, "test");
    (0, vitest_1.it)("should extract simplified information", async () => {
        const db = getKnex();
        await db.raw("create type test.some_composite_type as (id integer)");
        const result = await (0, extractCompositeType_1.default)(db, makePgType("some_composite_type"));
        const expected = {
    ...
    (0, vitest_1.it)("should fetch column comments", async () => {
        const db = getKnex();
        await db.raw("create type test.some_composite_type as (id integer, name text)");
        await db.raw("comment on column test.some_composite_type.id is 'id column'");
        const result = await (0, extractCompositeType_1.default)(db, makePgType("some_composite_type"));
        (0, vitest_1.expect)(result.attributes[0].comment).toBe("id column");
    });
    (0, vitest_1.it)("should handle domains, composite types, ranges and enums as well as arrays of those", async () => {
        ...
        await db.raw(`create type test.some_composite_type as (
        d test.some_domain,
        ...
        const result = await (0, extractCompositeType_1.default)(db, makePgType("some_composite_type"));
        const actual = R.map(R.pick(["name", "expandedType", "type", "isArray"]), result.attributes);
        const expected = [
            {
                name: "d",
