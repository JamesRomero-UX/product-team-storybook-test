 ...
 */
export async function* parseMultipart(message, boundary, options) {
...
export class MultipartParser {
    ...
    #boundaryLength;
    #state = 0 /* MultipartParserState.Start */;
    #buffer = null;
    #bodyController = null;
    #bodyLength = 0;
    ...
    async parse(message, handler) {
        if (this.#state !== 0 /* MultipartParserState.Start */) {
            this.#reset();
        }
        let results = [];
        ...
        if (this.#state !== 4 /* MultipartParserState.Done */) {
            throw new MultipartParseError('Unexpected end of stream');
        }
        await Promise.all(results);
    }
    #reset() {
        this.#state = 0 /* MultipartParserState.Start */;
        this.#buffer = null;
    ...
    #write(chunk, handler) {
        ...
        while (true) {
            if (this.#state === 3 /* MultipartParserState.Body */) {
                ...
                this.#state = 1 /* MultipartParserState.AfterBoundary */;
            }
            if (this.#state === 1 /* MultipartParserState.AfterBoundary */) {
                if (chunkLength - index < 2) {
                    this.#buffer = chunk.subarray(index);
                    break;
                }
                if (chunk[index] === 45 && chunk[index + 1] === 45) {
                    this.#state = 4 /* MultipartParserState.Done */;
                    break;
                }
                index += 2; // Skip \r\n after boundary
                this.#state = 2 /* MultipartParserState.Header */;
            }
            if (this.#state === 2 /* MultipartParserState.Header */) {
                ...
                if (headerEndIndex === -1) {
                    if (chunkLength - index > this.maxHeaderSize) {
                        throw new MultipartParseError(`Header size exceeds maximum allowed size of ${this.maxHeaderSize} bytes`);
                    }
                    this.#buffer = chunk.subarray(index);
                    break;
                }
                ...
                let part = new MultipartPart(header, new ReadableStream({
                    start: (controller) => {
                ...
                this.#state = 3 /* MultipartParserState.Body */;
                continue;
            }
            if (this.#state === 0 /* MultipartParserState.Start */) {
    ...
    #writeBody(chunk) {
        if (this.#bodyLength + chunk.length > this.maxFileSize) {
            let error = new MultipartParseError(`File size exceeds maximum allowed size of ${this.maxFileSize} bytes`);
