...
const setupContext = async (config, command) => {
    ...
    ((config) => {
        ...
        if (config.AUTH0_EXPERIMENTAL_EA) {
            logger_1.default.warn(`Experimental early access ${EA_FEATURES.length === 1
                ? 'feature [' + EA_FEATURES.join('') + '] is'
                : 'features [' + EA_FEATURES.join(',') + '] are'} enabled. These are in a pre-release state and may change in future release.`);
        }
        else if (EA_FEATURES.length > 0) {
            logger_1.default.info('To enable experimental early access features use --experimental_ea flag or set AUTH0_EXPERIMENTAL_EA=true in configuration JSON.');
        }
    })(config);
    const accessToken = await (async () => {
        ...
        const authClient = (() => {
            ...
            return new auth0_1.AuthenticationClient({
                domain: AUTH0_DOMAIN,
                clientId: AUTH0_CLIENT_ID,
                clientAssertionSigningKey: (0, fs_1.readFileSync)(AUTH0_CLIENT_SIGNING_KEY_PATH, 'utf8'),
