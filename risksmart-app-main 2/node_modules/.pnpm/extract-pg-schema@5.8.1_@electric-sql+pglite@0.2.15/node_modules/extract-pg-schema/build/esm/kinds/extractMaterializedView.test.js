...
describe("extractMaterializedView", () => {
    ...
    it("should extract simplified information", async () => {
        const db = getKnex();
        await db.raw("create materialized view test.some_materialized_view as select 1 as id");
        const result = await extractMaterializedView(db, makePgType("some_materialized_view"));
        const expected = {
    ...
    it("should fetch column comments", async () => {
        const db = getKnex();
        await db.raw("create materialized view test.some_materialized_view as select 1 as id");
        await db.raw("comment on column test.some_materialized_view.id is 'id column'");
        const result = await extractMaterializedView(db, makePgType("some_materialized_view"));
    ...
    it("should handle domains, composite types, ranges and enums as well as arrays of those", async () => {
        ...
        await db.raw("create materialized view test.some_materialized_view as select * from test.some_table");
        const result = await extractMaterializedView(db, makePgType("some_materialized_view"));
