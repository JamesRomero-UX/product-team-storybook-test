...
async function main(args) {
    ...
    const allSchemas = await (0, extractSchemas_1.default)({ ...connectionConfig, password });
    // Filter schemas after extracting them all, because we don't know
    // which schemas exist until we've extracted them.
    const schemas = Object.fromEntries(Object.entries(allSchemas).filter(([schemaName]) => schemaFilter(schemaName)));
    // eslint-disable-next-line no-console
    console.log(JSON.stringify(schemas, null, 2));
}
function parseArgs(args) {
    const { values, positionals } = node_util_1.default.parseArgs({
        args,
        options: {
            help: { type: "boolean" },
...
function showHelp() {
    console.error(`Usage: ${process.argv[1]} [options] [DBNAME]
