...
const extractView_1 = __importDefault(require("./extractView"));
...
(0, vitest_1.describe)("extractView", () => {
    const [getKnex, databaseName] = (0, useTestKnex_1.default)();
    (0, useSchema_1.default)(getKnex, "test");
    (0, vitest_1.it)("should extract simplified as well as full information_schema information", async () => {
        const db = getKnex();
        await db.raw("create view test.some_view as select 1 as id");
        const result = await (0, extractView_1.default)(db, makePgType("some_view"));
        const expected = {
            name: "some_view",
            schemaName: "test",
            kind: "view",
    ...
    (0, vitest_1.it)("should fetch column comments", async () => {
        const db = getKnex();
        await db.raw("create view test.some_view as select 1 as id");
    ...
    (0, vitest_1.it)("should handle domains, composite types, ranges and enums as well as arrays of those", async () => {
        const db = getKnex();
        ...
        const result = await (0, extractView_1.default)(db, makePgType("some_view"));
        const actual = R.map(R.pick(["name", "expandedType", "type", "isArray"]), result.columns);
        const expected = [
            {
                name: "d",
    ...
    (0, vitest_1.it)("should report the correct source", async () => {
        const db = getKnex();
        await db.raw("create table test.some_table (id integer not null)");
        await db.raw("create view test.some_view as select * from test.some_table");
        const result = await (0, extractView_1.default)(db, makePgType("some_view"));
        (0, vitest_1.expect)(result.columns[0].type.fullName).toBe("pg_catalog.int4");
        (0, vitest_1.expect)(result.columns[0].source).toEqual({
            schema: "test",
            table: "some_table",
            column: "id",
        });
    });
    (0, vitest_1.it)("should extract view options", async () => {
        const db = getKnex();
    ...
    `);
        const result = await (0, extractView_1.default)(db, makePgType("some_view"));
        (0, vitest_1.expect)(result.options).toEqual({
            checkOption: "local",
            securityBarrier: true,
            securityInvoker: true,
        });
    });
    (0, vitest_1.it)("should handle views without explicit options", async () => {
        const db = getKnex();
        await db.raw("create view test.some_view as select 1 as id");
        const result = await (0, extractView_1.default)(db, makePgType("some_view"));
        (0, vitest_1.expect)(result.options).toEqual({
