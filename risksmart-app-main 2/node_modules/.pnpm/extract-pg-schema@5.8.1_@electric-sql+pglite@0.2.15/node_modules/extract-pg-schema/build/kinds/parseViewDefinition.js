...
const createPgQuery = async () => {
    ...
    const pgQueryModule = await eval('import("pg-query-emscripten/pg_query_wasm.js")');
...
defaultSchema, aliases) {
    if (selectAst.larg && selectAst.rarg) {
        ...
        const lLocalAliases = Object.fromEntries(lArgAliases.map(({ schemaname, relname, alias }) => [
            alias.aliasname,
            { schema: schemaname, table: relname },
        ]));
        const rArgAliases = findAliasDefinitions(selectAst.rarg);
        const rLocalAliases = Object.fromEntries(rArgAliases.map(({ schemaname, relname, alias }) => [
            alias.aliasname,
            { schema: schemaname, table: relname },
        ]));
        return [
    ...
    selectAst.fromClause.forEach((fromClause) => {
        ...
        selectTargets.forEach((selectTarget) => {
            ...
            const vals = selectTarget.val
                ...
                : [];
            const fields = (vals
                ?.map((val) => val.ColumnRef?.fields?.map((field) => field.String?.sval))
                .flat()
                .filter(Boolean) ?? []);
...
function findAliasDefinitionsInJoinExpr(joinExpr) {
    const result = [];
...
function findAliasDefinitions(node) {
    // const aliasDefinitions = jp.query(
    //   ast,
    //   "$.stmt.SelectStmt.fromClause..[?(@.alias)]",
    // );
    const aliasDefinitions = node.fromClause?.flatMap((clause) => {
        const result = [];
 ...
 */
function resolveCteThroughChain(viewRef, cteAliases, visited = new Set()) {
...
async function parseViewDefinition(selectStatement, defaultSchema) {
    ...
    for (const cte of withClauses) {
        if (cte.CommonTableExpr) {
            ...
            const cteLocalAliases = Object.fromEntries(cteQueryAliases.map(({ schemaname, relname, alias }) => [
                alias.aliasname,
                { schema: schemaname, table: relname },
            ]));
            ...
            const cteColumns = parseSelectStmt(cteQuery, defaultSchema, mergedAliases);
            cteAliases[cteName] = cteColumns;
        }
    }
    // Find aliases in the main SELECT
    const mainSelectAliases = findAliasDefinitions(selectAst);
    const mainLocalAliases = Object.fromEntries(mainSelectAliases.map(({ schemaname, relname, alias }) => [
        alias.aliasname,
        { schema: schemaname, table: relname },
    ]));
    const mergedMainAliases = { ...aliases, ...mainLocalAliases };
    const primaryViewReferences = parseSelectStmt(selectAst, defaultSchema, mergedMainAliases);
    // Resolve each view reference through the CTE chain
    const viewReferences = primaryViewReferences.map((viewReference) => resolveCteThroughChain(viewReference, cteAliases));
    return viewReferences;
}
