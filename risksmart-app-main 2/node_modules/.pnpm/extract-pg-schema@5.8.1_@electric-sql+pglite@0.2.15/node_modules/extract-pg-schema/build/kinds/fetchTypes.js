...
const fetchTypes = async (db, schemaNames) => {
    ...
    `COALESCE(
        ...
        .whereIn("pg_type.typtype", ["c", ...Object.keys(PgType_1.typeKindMap)])
        .whereIn("pg_namespace.nspname", schemaNames);
    const procQuery = db
        .select("proname as name", "nspname as schemaName", db.raw(`case prokind
        when 'f' then 'function'
        when 'p' then 'procedure'
        when 'a' then 'aggregate'
        when 'w' then 'window'
        end as kind`), db.raw("obj_description(pg_proc.oid) as comment"))
