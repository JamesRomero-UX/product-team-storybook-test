  ...
  for (const operation of operations) {
    ...
    for (const variableName of operationOrFragmentVariables) {
      ...
      if (variableDef != null) {
        ...
        const variableDef2 = delegationContext.info?.operation.variableDefinitions?.find(
          (varDef) => varDef.variable.name.value === variableName
        );
        if (variableDef2 != null) {
          variableDefinitions.push(variableDef2);
        }
      }
    }
      ...
      if (existingArgNode?.value.kind === graphql.Kind.VARIABLE) {
        ...
        if (varValue === argValue) {
          ...
          const varDef = info?.operation.variableDefinitions?.find(
            (varDef2) => varDef2.variable.name.value === varName
          );
          if (varDef) {
