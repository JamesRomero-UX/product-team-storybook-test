...
exports.isSdkClient = exports.addUserAgentMiddleware = exports.customUserAgentMiddleware = void 0;
...
const customUserAgentMiddleware = (feature) => {
    return (next) => async (args) => {
        ...
        if (existingUserAgent.includes('PT/NO-OP')) {
            const featureSpecificUserAgent = existingUserAgent.replace('PT/NO-OP', `PT/${feature}/${version_js_1.PT_VERSION} PTEnv/${EXEC_ENV}`);
        ...
        args.request.headers['user-agent'] =
            existingUserAgent === ''
                ? `PT/${feature}/${version_js_1.PT_VERSION} PTEnv/${EXEC_ENV}`
                : `${existingUserAgent} PT/${feature}/${version_js_1.PT_VERSION} PTEnv/${EXEC_ENV}`;
        return await next(args);
    };
};
